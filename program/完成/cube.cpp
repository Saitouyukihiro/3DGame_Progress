...
bool CollisionCude(D3DXVECTOR3 *pos, D3DXVECTOR3 *posold, D3DXVECTOR3 *move, float fRadiou)
{
	...
	D3DXVECTOR3 norvecXZ = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
	D3DXVECTOR3 norvecY = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
	...
	D3DXVECTOR3 longnorvec = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
	D3DXVECTOR3 askvec = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
	LPDIRECT3DDEVICE9	pDevice; //デバイスのポインタ
	float inner;
	float fask[4];
	float faskold[4];
	...
	for (int nCntCube = 0; nCntCube < MAX_CUBE; nCntCube++, pCube++)
	{
		if (g_aCube[nCntCube].bUse == true)
		{
			...
			//奥の壁
			if (fask[0] < 0.0f && faskold[0] >= 0.0f)
			{
				norvecXZ = D3DXVECTOR3(pCube->vertexmtxWold[4]._41 - pCube->vertexmtxWold[5]._41,
					pCube->vertexmtxWold[4]._42 - pCube->vertexmtxWold[5]._42,
					pCube->vertexmtxWold[4]._43 - pCube->vertexmtxWold[5]._43);

				norvecY = D3DXVECTOR3(pCube->vertexmtxWold[1]._41 - pCube->vertexmtxWold[5]._41,
					pCube->vertexmtxWold[1]._42 - pCube->vertexmtxWold[5]._42,
					pCube->vertexmtxWold[1]._43 - pCube->vertexmtxWold[5]._43);
			}

			//右
			else if (fask[1] < 0.0f && faskold[1] >= 0.0f)
			{
				norvecXZ = D3DXVECTOR3(pCube->vertexmtxWold[5]._41 - pCube->vertexmtxWold[7]._41,
					pCube->vertexmtxWold[5]._42 - pCube->vertexmtxWold[7]._42,
					pCube->vertexmtxWold[5]._43 - pCube->vertexmtxWold[7]._43);

				norvecY = D3DXVECTOR3(pCube->vertexmtxWold[3]._41 - pCube->vertexmtxWold[7]._41,
					pCube->vertexmtxWold[3]._42 - pCube->vertexmtxWold[7]._42,
					pCube->vertexmtxWold[3]._43 - pCube->vertexmtxWold[7]._43);
			}

			//前
			else if (fask[2] < 0.0f && faskold[2] >= 0.0f)
			{
				norvecXZ = D3DXVECTOR3(pCube->vertexmtxWold[7]._41 - pCube->vertexmtxWold[6]._41,
					pCube->vertexmtxWold[7]._42 - pCube->vertexmtxWold[6]._42,
					pCube->vertexmtxWold[7]._43 - pCube->vertexmtxWold[6]._43);

				norvecY = D3DXVECTOR3(pCube->vertexmtxWold[2]._41 - pCube->vertexmtxWold[6]._41,
					pCube->vertexmtxWold[2]._42 - pCube->vertexmtxWold[6]._42,
					pCube->vertexmtxWold[2]._43 - pCube->vertexmtxWold[6]._43);
			}

			//左側
			else if (fask[2] < 0.0f && faskold[2] >= 0.0f)
			{
				norvecXZ = D3DXVECTOR3(pCube->vertexmtxWold[6]._41 - pCube->vertexmtxWold[4]._41,
					pCube->vertexmtxWold[6]._42 - pCube->vertexmtxWold[4]._42,
					pCube->vertexmtxWold[6]._43 - pCube->vertexmtxWold[4]._43);

				norvecY = D3DXVECTOR3(pCube->vertexmtxWold[0]._41 - pCube->vertexmtxWold[4]._41,
					pCube->vertexmtxWold[0]._42 - pCube->vertexmtxWold[4]._42,
					pCube->vertexmtxWold[0]._43 - pCube->vertexmtxWold[4]._43);
			}

			//外積
			D3DXVec3Cross(&norvec, &norvecXZ, &norvecY);
			//正規化
			D3DXVec3Normalize(&norvec, &norvec);

			movevec = D3DXVECTOR3(pos->x - posold->x, pos->y - posold->y, pos->z - posold->z);

			//内積
			inner = (-movevec.x * norvec.x) + (-movevec.y * norvec.y) + (-movevec.z * norvec.z);

			longnorvec = D3DXVECTOR3(norvec.x * inner, norvec.y * inner, norvec.z * inner);

			//
			if ((fask[0] < 0.0f && fask[1] < 0.0f && fask[2] < 0.0f && fask[3] < 0.0f) &&
				(faskold[0] >= 0.0f || faskold[1] >= 0.0f || faskold[2] >= 0.0f || faskold[3] >= 0.0f))
			{
				askvec = D3DXVECTOR3(movevec.x + longnorvec.x, movevec.y + longnorvec.y, movevec.z + longnorvec.z);
