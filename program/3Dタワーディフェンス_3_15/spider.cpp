...
#endif // DEBUG
			SetPointShadow(g_aSpider[nCntSpider].nIdex, D3DXVECTOR3(g_aSpider[nCntSpider].pos.x, 0.0f, g_aSpider[nCntSpider].pos.z), 50.0f, 50.0f);
		}
	}
}
...
void SetSpider(D3DXVECTOR3 pos, D3DXVECTOR3 rot)
{
	...
	for (int nCntSpider = 0; nCntSpider < MAX_SPIDER_MODEL; nCntSpider++)
	{
		if (g_aSpider[nCntSpider].bUse == false)
		{
			...
			g_aSpider[nCntSpider].nIdex = SetShadow(D3DXVECTOR3(g_aSpider[nCntSpider].pos.x, 0.0f, g_aSpider[nCntSpider].pos.z), 50.0f, 50.0f);
...
void SpiderLooking(D3DXVECTOR3 pos)
{
	int nSide;
	for (int nCntSpider = 0; nCntSpider < MAX_SPIDER_MODEL; nCntSpider++)
	{
		if (g_aSpider[nCntSpider].bUse == true)
		{
			if (pos.x >= g_aSpider[nCntSpider].pos.x - g_aSpider[nCntSpider].fLookigX &&
				pos.x <= g_aSpider[nCntSpider].pos.x + g_aSpider[nCntSpider].fLookigX &&
				pos.z <= g_aSpider[nCntSpider].pos.z + g_aSpider[nCntSpider].fLookigZ &&
				pos.z >= g_aSpider[nCntSpider].pos.z - g_aSpider[nCntSpider].fLookigZ &&
				g_aSpider[nCntSpider].aLook == SPIDER_STATE_NOLOOK)
			{
				g_aSpider[nCntSpider].posdfrc = pos - g_aSpider[nCntSpider].pos;
			}
			if (g_aSpider[nCntSpider].posdfrc.x == 0.0f &&
				g_aSpider[nCntSpider].posdfrc.z == 0.0f)
			{
				g_aSpider[nCntSpider].aState = SPIDER_STATE_ATTACK;
				g_aSpider[nCntSpider].aLook = SPIDER_STATE_NOLOOK;
			}
			...
			{
				g_aSpider[nCntSpider].aLook = SPIDER_STATE_LOOK;
			}

			if (g_aSpider[nCntSpider].posdfrc.x >= 0.0f)
			{
				g_aSpider[nCntSpider].move.x = 1.0f;
			}
			else if (g_aSpider[nCntSpider].posdfrc.x <= 0.0f)
			{
				g_aSpider[nCntSpider].move.x = -1.0f;
			}
			if (g_aSpider[nCntSpider].posdfrc.z >= 0.0f)
			{
				g_aSpider[nCntSpider].move.z = 1.0f;
			}
			else if (g_aSpider[nCntSpider].posdfrc.z <= 0.0f)
			{
				g_aSpider[nCntSpider].move.z = -1.0f;
			}
		}
