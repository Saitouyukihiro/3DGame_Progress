...
HRESULT InitFloo(void)
{
	...
	D3DXCreateTextureFromFile(	pDevice,
								"data/Texture/floo_000.png",			//テクスチャ
								&g_pTextureField[0]);
...
//床の当たり判定
//---------------------------
bool ColisionFloo(D3DXVECTOR3 *pos, D3DXVECTOR3 *posold, D3DXVECTOR3 *move, float fWidth, float fHeight, float fDepht, int nType)
{
	...
	for (int nCntFloo = 0; nCntFloo < MAX_FLOO; nCntFloo++, pFloo++)
	{
		if (pFloo->bUse == true)
		{
			if (pos->x <= pFloo->pos.x + pFloo->fWidth / 2 &&
				pos->x >= pFloo->pos.x - pFloo->fWidth / 2 &&
				pos->z <= pFloo->pos.z + pFloo->fDepht / 2 &&
				pos->z >= pFloo->pos.z - pFloo->fDepht / 2)
			{
				if (posold->y >= pFloo->pos.y &&
					pos->y <= pFloo->pos.y)
				{
					move->y = 0.0f;
					pos->y = pFloo->pos.y;	//地面に合わせる
					bLand = true;			//地面についてる
				}
			}
		}
