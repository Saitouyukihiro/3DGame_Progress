...
HRESULT InitBullet(void)
{
	...
	D3DXCreateTextureFromFile(	pDevice,
								"data/Texture/bullet_000.png",			//テクスチャ
								&g_pTextureBullet);
	//頂点バッファの生成
...
void UpdateBullet(void)
{
	for (int nCntBullet = 0; nCntBullet < MAX_BULLET; nCntBullet++)
	{
		if (g_aBullet[nCntBullet].bUse == true)
		{
			...
			if (CollisionSpider(&g_aBullet[nCntBullet].pos, &g_aBullet[nCntBullet].posold, &g_aBullet[nCntBullet].move, g_aBullet[nCntBullet].fWidth, g_aBullet[nCntBullet].fHeight, COLLISION_BULLET) == true)
			{
			...
			if (g_aBullet[nCntBullet].pos.x > 2000)
			{
				g_aBullet[nCntBullet].bUse = false;
			}
			else if (g_aBullet[nCntBullet].pos.x < -2000)
			{
				g_aBullet[nCntBullet].bUse = false;
			}
			if (g_aBullet[nCntBullet].pos.z > 2000)
			{
				g_aBullet[nCntBullet].bUse = false;
			}
			else if (g_aBullet[nCntBullet].pos.z < -2000)
			{
				g_aBullet[nCntBullet].bUse = false;
			}
			if (g_aBullet[nCntBullet].pos.y < -100)
			{
				g_aBullet[nCntBullet].bUse = false;
			}
		}
