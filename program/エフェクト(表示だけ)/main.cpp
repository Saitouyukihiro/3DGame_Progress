...
HRESULT Init(HINSTANCE hInstance, HWND hWnd, BOOL bWindow)
{
	...
	D3DXCreateFont(g_pD3DDevice, 18, 0, 0, 0,//大きさ
	...
	SetBillboard(D3DXVECTOR3(0.0f, 0.0f, -120.0f), 50.0f, 30.0f, 0);
	SetBillboard(D3DXVECTOR3(100.0f, 0.0f, -120.0f), 50.0f, 30.0f, 0);


	InitEffect();
	SetEffect(pPlayer->pos, D3DXCOLOR(255, 0, 0, 255), 20, 20, EFFECTTYTPE_PLAYER, 20);
...
void DrawTxT(void)
{
	...
	//nNum += sprintf(&aStr[nNum], "モデルX軸最大、最小:(%.1f : %.1f)\n", pPlayer->VtxMaxModel.x, pModel->VtxMinModel.x);
	//nNum += sprintf(&aStr[nNum], "モデルY軸最大、最小:(%.1f : %.1f)\n", pPlayer->VtxMaxModel.y, pModel->VtxMinModel.y);
	//nNum += sprintf(&aStr[nNum], "モデルZ軸最大、最小:(%.1f : %.1f)\n", pPlayer->VtxMaxModel.z, pModel->VtxMinModel.z);
