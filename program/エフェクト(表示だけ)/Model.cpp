...
//マクロ
#define MOVE (5.0f)
#define BULLET_COOLDOWN (2) //弾の発射間隔
...
void UpdateModel(void)//追加の可能性があるため項目は作っとく
{
	...
	else if (GetKeyboardPress(DIK_DOWN) == true)
	{
		...
		/*if (g_Model.rot.y * -1 >=  pCamera->rot.y)
		{
			g_Model.rot.y += 0.1;
			if (g_Model.rot.y * -1 > pCamera->rot.y * -1)
			{
				g_Model.rot.y  = pCamera->rot.y * -1;
			}
		}
		if (g_Model.rot.y * -1 <= pCamera->rot.y)
		{
	...
	if (GetKeyboardPress(DIK_LEFT) == true)
	{
		...
		//g_Player.rot.y = (D3DX_PI * -0.5) + pCamera->rot.y;
	}
	...
	UpdateShadow();
	SetEffect(g_Player.pos, D3DXCOLOR(255, 0, 0, 255), 20, 20, EFFECTTYTPE_PLAYER, 20);
