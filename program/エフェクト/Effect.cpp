...
void InitEffect(void)
{
	...
	//テクスチャの読み込み
	D3DXCreateTextureFromFile(pDevice, "date/TEXTURE/Effect000.jpg", &g_pTextureEffect[EFFECTTYPE_PLAYER]);

	D3DXCreateTextureFromFile(pDevice, "date/TEXTURE/Effect000.jpg", &g_pTextureEffect[EFFECTTYPE_FOLL_ST]);
	D3DXCreateTextureFromFile(pDevice, "date/TEXTURE/Effect000.jpg", &g_pTextureEffect[EFFECTTYPE_FOLL_L]);
	D3DXCreateTextureFromFile(pDevice, "date/TEXTURE/Effect000.jpg", &g_pTextureEffect[EFFECTTYPE_FOLL_R]);

	D3DXCreateTextureFromFile(pDevice, "date/TEXTURE/Effect000.jpg", &g_pTextureEffect[EFFECTTYPE_UP_ST]);
	D3DXCreateTextureFromFile(pDevice, "date/TEXTURE/Effect000.jpg", &g_pTextureEffect[EFFECTTYPE_UP_L]);
	D3DXCreateTextureFromFile(pDevice, "date/TEXTURE/Effect000.jpg", &g_pTextureEffect[EFFECTTYPE_UP_R]);
...
void UpdateEffect(void)
{
	...
	for (int nCntEffect = 0; nCntEffect < MAX_EFFECT; nCntEffect++)
	{
		if (g_Effect[nCntEffect].bUse == true)
		{
			...
			g_Effect[nCntEffect].fWhidth++;
			g_Effect[nCntEffect].fHight--;
		...
		if (g_Effect[nCntEffect].ntype == EFFECTTYPE_FOLL_ST)
		{
			g_Effect[nCntEffect].move.y -= 2;
			g_Effect[nCntEffect].pos.y = g_Effect[nCntEffect].move.y;
		}

		if (g_Effect[nCntEffect].ntype == EFFECTTYPE_FOLL_L)
		{
			g_Effect[nCntEffect].move.y -= 2;
			g_Effect[nCntEffect].pos.y = g_Effect[nCntEffect].move.y;
			
			g_Effect[nCntEffect].pos.x -= cosf(pCamera->rot.y) * 2;
			g_Effect[nCntEffect].pos.z += sinf(pCamera->rot.y) * 2;
		}

		if (g_Effect[nCntEffect].ntype == EFFECTTYPE_FOLL_R)
		{
			g_Effect[nCntEffect].move.y -= 2;
			g_Effect[nCntEffect].pos.y = g_Effect[nCntEffect].move.y;
		...
		if (g_Effect[nCntEffect].ntype == EFFECTTYPE_UP_ST)
		{
			g_Effect[nCntEffect].move.y += 2;

			g_Effect[nCntEffect].pos.y = g_Effect[nCntEffect].move.y;
		}
		if (g_Effect[nCntEffect].ntype == EFFECTTYPE_UP_L)
		{
			g_Effect[nCntEffect].move.y += 2;
			g_Effect[nCntEffect].pos.y = g_Effect[nCntEffect].move.y;
		...
		if (g_Effect[nCntEffect].ntype == EFFECTTYPE_UP_R)
		{
			g_Effect[nCntEffect].move.y += 2;
			g_Effect[nCntEffect].pos.y = g_Effect[nCntEffect].move.y;
...
void SetEffect(D3DXVECTOR3 pos, D3DXCOLOR color, float fWhidth, float fHight, int ntype, int nLife)
{
	...
	for (nCntEffect = 0; nCntEffect < MAX_EFFECT; nCntEffect++)
	{
		if (g_Effect[nCntEffect].bUse == false)
		{
			...
			g_Effect[nCntEffect].color = color;
			...
			//頂点の色
			pVtx[0].col = g_Effect[nCntEffect].color; 
			pVtx[1].col = g_Effect[nCntEffect].color;
			pVtx[2].col = g_Effect[nCntEffect].color;
			pVtx[3].col = g_Effect[nCntEffect].color;
