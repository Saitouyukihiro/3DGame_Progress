...
HRESULT InitCamera(void)
{
	...
	g_Camera[2].PosV = (D3DXVECTOR3(1000.0f, 1500.0f, -500.0f));
...
{
	Model * pModel;	//モデルの構造体

	pModel = GetModel();	//モデルの情報
	...
	//TPS視点
	//---------------------------
	g_Camera[1].PosR = pModel->PosModel;

	g_Camera[1].PosR.x = pModel->PosModel.x + sinf(g_Camera[1].Rot.y + 10.0f) * g_Camera[1].Long;
	g_Camera[1].PosR.z = pModel->PosModel.z + cosf(g_Camera[1].Rot.y + 10.0f) * g_Camera[1].Long;
	g_Camera[1].PosR.y = pModel->PosModel.y + 5.0f;

	//---------------------------
	//FPS視点
	//---------------------------
	g_Camera[0].PosV = pModel->PosModel;

	g_Camera[0].PosV.x = pModel->PosModel.x;
	g_Camera[0].PosV.z = pModel->PosModel.z + pModel->vtxMaxModel.z;
	g_Camera[0].PosV.y = pModel->PosModel.y + 25.0f;
	...
	for (int nCntCm = 0; nCntCm < MAX_CAMERA; nCntCm++)
	{
		if (g_Camera[nCntCm].bUse == true)
		{
			...
			{
				g_Camera[nCntCm].PosR.y += g_Camera[nCntCm].Poimove.y * 3.0f;
			}
