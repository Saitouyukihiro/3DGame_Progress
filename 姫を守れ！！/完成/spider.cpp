...
void UpdateSpider(void)
{
	...
	for (int nCntSpider = 0; nCntSpider < MAX_SPIDER_MODEL; nCntSpider++)
	{
		...
		{
			g_aSpider[nCntSpider].posold = g_aSpider[nCntSpider].pos;
...
#endif // DEBUG
			if (CollisionCude(&g_aSpider[nCntSpider].pos, &g_aSpider[nCntSpider].posold, &g_aSpider[nCntSpider].move, 50.0f / 5) == true)
			{
