...
//移動範囲
	...
	if (g_aPlayer.pos.x > PLAYER_X)
	{
		pCamera->PosV.x = PLAYER_X;
	}
	else if (g_aPlayer.pos.x < -PLAYER_X)
	{
		pCamera->PosV.x = -PLAYER_X;
	}
	if (g_aPlayer.pos.z > PLAYER_Z)
	{
		pCamera->PosV.z = PLAYER_Z;
	}
	else if (g_aPlayer.pos.z < -PLAYER_Z)
	{
		pCamera->PosV.z = -PLAYER_Z;
	}
